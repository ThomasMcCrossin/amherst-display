...
cp "$SRC/scorebug_profiles.py" "$SRC/scorebug_detect.py" "$SRC/drive_config.py" "$DEST/"
...
grep -rl "highlight_extractor" "$DEST/hockey_extractor" "$DEST/scorebug_detect.py" "$DEST/tests" \
...
if grep -rn -I -E "/home/[a-z]+/|@curlys\.ca|sk-[a-f0-9]{32}|BEGIN (RSA )?PRIVATE KEY" \
        "$DEST/hockey_extractor" "$DEST/scorebug_profiles.py" "$DEST/scorebug_detect.py" "$DEST/drive_config.py"; then
